...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load(model_path, map_location=None)["state_dict"], strict=True
    )
    model = model.to("cuda")
    model.eval()
    ...
    tifffile.imwrite(
        f"test_results/mix_test5/Mix_10_00{i}_resized_labelled.tif",
        logits.to(torch.uint8).cpu().detach().numpy(),
    )
