...
def create_mix(bin, i, mode):
    ...
    tifffile.imwrite(
        _PATH_DATA
        + f"/synthetic_mixed_256/{mode}/{subfolder}/mix_{str(i).zfill(5)}.tif",
        new_mix,
    )
    tifffile.imwrite(
        _PATH_DATA
        + f"/synthetic_mixed_256/{mode}/{subfolder}/label_{str(i).zfill(5)}.tif",
        new_mix_label,
    )
...
if __name__ == "__main__":
    ...
    Parallel(n_jobs=-1)(
        delayed(generate_mix)(df, df_noise, i, "train")
        for i in tqdm(range(5000), unit="image", desc="creating mixed images")
    )
    Parallel(n_jobs=-1)(
        delayed(generate_mix)(df, df_noise, i, "test")
        for i in tqdm(range(1000), unit="image", desc="creating mixed images")
    )
    Parallel(n_jobs=-1)(
        delayed(generate_mix)(df, df_noise, i, "validation")
        for i in tqdm(range(1000), unit="image", desc="creating mixed images")
    )
