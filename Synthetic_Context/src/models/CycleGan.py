...
class CycleGan(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict({'train/gen_loss': genLoss.item()},
                      on_step=True,
                      on_epoch=True)
        ...
        self.log_dict({'train/dis_loss': disLoss.item()},
                      on_step=True,
                      on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        genLoss = self.generator_step(imgA, imgB)
        self.log_dict({'val/gen_loss': genLoss.item()},
                          on_step=False,
                          on_epoch=True)
        disLoss = self.discriminator_step(imgA, imgB)
        self.log_dict({'val/dis_loss': disLoss.item()},
                          on_step=False,
                          on_epoch=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        genLoss = self.generator_step(imgA, imgB)
        self.log_dict({'test/gen_loss': genLoss.item()},
                          on_step=False,
                          on_epoch=True)
        disLoss = self.discriminator_step(imgA, imgB)
        self.log_dict({'test/dis_loss': disLoss.item()},
                          on_step=False,
                          on_epoch=True)
